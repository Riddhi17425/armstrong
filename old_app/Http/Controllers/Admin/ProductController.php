...
class ProductController extends Controller
{
    ...
    public function ProductStore(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'hero_video_source'     => [Rule::requiredIf(fn() => $request->media_type === 'video'), 'nullable', 'in:upload,youtube'],
            'hero_youtube_link'     => [Rule::requiredIf(fn() => $request->media_type === 'video' && $request->hero_video_source === 'youtube'), 'nullable', 'url'],
            'hero_thumbnails_image' => [Rule::requiredIf(fn() => $request->media_type === 'video'), 'nullable', 'image', 'mimes:jpg,jpeg,png,webp', 'max:2048'],
        ...
        try {
            ...
            if ($request->media_type === 'video') {
                $heroVideoPath = $request->hero_video_source === 'youtube'
                    ? $request->hero_youtube_link
                    : $request->hero_uploaded_video_path;
            }

            $heroThumbnail = '';
            if ($request->hasFile('hero_thumbnails_image')) {
                $heroImage     = $request->file('hero_thumbnails_image');
                $heroImageName = 'hero_' . time() . '_' . $heroImage->getClientOriginalName();
                $heroImage->move(public_path('admin/product/videos/images'), $heroImageName);
                $heroThumbnail = 'public/admin/product/videos/images/' . $heroImageName;
            }
            ...
            $product = ProductMaster::create([
                ...
                'hero_video_source'    => $request->media_type === 'video' ? $request->hero_video_source : null,
                'hero_video'           => $heroVideoPath,
                'hero_thumbnail_image' => $heroThumbnail,
    ...
    public function getProductData()
    {
        ...
        return DataTables::of($product)
            ...
            ->addColumn('image', function ($row) {
                if ($row->media_type === 'video' && $row->hero_thumbnail_image) {
                    return $row->hero_thumbnail_image;
                }
    ...
    public function UpdateProduct(Request $request, $id)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'hero_video_source'     => [Rule::requiredIf(fn() => $request->media_type === 'video'), 'nullable', 'in:upload,youtube'],
            'hero_youtube_link'     => [Rule::requiredIf(fn() => $request->media_type === 'video' && $request->hero_video_source === 'youtube'), 'nullable', 'url'],
            'hero_thumbnails_image' => 'nullable|image|mimes:jpg,jpeg,png,webp|max:2048',
        ...
        try {
            ...
            $heroVideoPath = $product->hero_video;
            if ($request->media_type === 'video') {
                $heroVideoPath = $request->hero_video_source === 'youtube'
                    ? $request->hero_youtube_link
                    : ($request->hero_uploaded_video_path ?: $product->hero_video);
            }

            $heroThumbnail = $product->hero_thumbnail_image;
            if ($request->hasFile('hero_thumbnails_image')) {
                if ($heroThumbnail && file_exists(public_path(str_replace('public/', '', $heroThumbnail)))) {
                    unlink(public_path(str_replace('public/', '', $heroThumbnail)));
                }
                $heroImage     = $request->file('hero_thumbnails_image');
                $heroImageName = 'hero_' . time() . '_' . $heroImage->getClientOriginalName();
                $heroImage->move(public_path('admin/product/videos/images'), $heroImageName);
